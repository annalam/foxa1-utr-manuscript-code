...
//use perf_profiler::PerfProfiler;
//use perf_profiler::TaskProfiler;
use perf_profiler::DisabledTimedTask;

const MAX_READ_LEN: usize = 5000;
const WINDOW_LEN: usize = 100000;
const USAGE: &str = "
Usage:
...
Options:
  ...
  --min-mapq=N        Minimum average MAPQ for base substitutions [default: 0]
  --no-index    	    Do not use bam index files (.bam.bai) in processing even 
                      if available.                    
  --count-duplicates  Count duplicate reads
...
impl Substitution {
	pub fn new() -> Substitution {
		Substitution { reads: 0, sidedness: 0.0, baseq: 0.0, mapq: 0.0 }
	}
}

#[derive(Clone)]
struct Indel {
	//IString, when available in stable releases
...
// Pileup simply be a SmallVec with room for 2 or 3 Mutations objects.
#[derive(Clone)]
struct Pileup {
	total: u32,
	acgt: [Substitution; 4],
	indels: Vec<Indel> //Array?
}
...
pub fn main() {
	...
	let alt_reads: u32 = args.get_str("--alt-reads").parse().unwrap();
	let alt_frac: f32 = args.get_str("--alt-frac").parse().unwrap();
	let min_subs_mapq: i32 = args.get_str("--min-mapq").parse().unwrap();
	let count_duplicates = args.get_bool("--count-duplicates");
	let count_discordant = args.get_bool("--count-discordant");
	let genome_region = args.get_str("--region");
	let ignore_bam_index_files = args.get_bool("--no-index");
	...
	// Replace TimedTask:: with DisabledTimedTask:: to disable performance reporting
	let mut t_chr =     DisabledTimedTask::new( "Loading ref chromosomes", "");
	let mut t_bam =     DisabledTimedTask::new( "Reading bam files", "");
	let mut t_pileups = DisabledTimedTask::new( "Processing pileups", "");
	//let mut t_build_indels = DisabledTimedTask::new( "Building indels", "Processing pileups");
	//let mut t_print_indels = DisabledTimedTask::new( "Printing indels", "Processing pileups");
	//let mut t_print_bs =     DisabledTimedTask::new( "Print base substitutions", "Processing pileups");
	let mut t_drain =   DisabledTimedTask::new( "Draining window", "");
	let mut t_total =   DisabledTimedTask::new( "Test total", "");  // Control measurement, expect ~100%
	let mut t_total2 =  DisabledTimedTask::new( "Test total2", ""); // Control measurement, expect ~100%
	let mut t_total3 =  DisabledTimedTask::new( "Test total3", ""); // Control measurement, expect ~100%
		...
		'window_loop: loop {  
			...
			'bam_loop: for bam_index in 0..n_bams {
				...
				'reads_loop: loop {
					...
					assert!( read_length < MAX_READ_LEN );
					...
					for s in cigar.iter() {						
						...
						match *s {
							...
							// Cigar::Equal and Cigar::Diff are newer additions and not supported
							// by all aligners
							Cigar::Match(len) | Cigar::Equal(len) | Cigar::Diff(len) => {
								for _k in 0..len {								
									...
									if acgt_idx < 4 {
										...
										acgt.baseq = (acgt.baseq * n_reads_before + (qual[ seq_idx] as f32)) / n_reads;
										//eprintln!( "bq: {} seq: {}", read.qual().len(), seq.len());
									}
									else {
										prior_n = true;
									}
							...
							Cigar::Ins(len) => {
								...
								// TODO: Could avoid allocations here by reusing
								// buffer, only allocating for new indels.							
								...
								let mut base_qual_avg = 0.0f32; //Avg for the inserted bases
								...
								if allele.contains("N") { continue; } // Discard insertions with any Ns

								// Distance from nearest read end
								let end_distance = ::std::cmp::min( seq_idx-(len as usize), (seq.len()-1)-seq_idx ) as f32;
								let pileup = &mut pileups[ bam_index][ pileup_idx-1];

								count_indel( pileup, &allele, end_distance, read.mapq() as f32, base_qual_avg);
							...
							Cigar::Del(len) => {
								...
								// Distance from nearest read end
								let end_distance = ::std::cmp::min( seq_idx, (seq.len()-1)-seq_idx ) as f32;								
								count_indel( &mut pileup, &allele, end_distance, read.mapq() as f32, qual[ seq_idx] as f32 );
								// Indels are assigned to the coordinate of the previous MATCHing base
								// If the previous MATCHing base was an N, it has not been added to 
								// pile.total and needs to be added here								
								if prior_n { pileup.total += 1; }
							...
							Cigar::RefSkip(_len) => error!("Unexpected CIGAR type: N"),
							Cigar::SoftClip(_len) => error!("Unexpected CIGAR type: S"),
							Cigar::HardClip(_len) => error!("Unexpected CIGAR type: H"),
							Cigar::Pad(_len) => error!("Unexpected CIGAR type: P"),
							Cigar::Back(_len) => error!("Unexpected CIGAR type: 9")
						};
			...
			// If no reads begin or extend into the current window, pileups are empty
	...
	// Processing done, report stats
	t_total.end();
	t_total2.end();
	t_total3.end();
	eprintln!("INFO: Processing took {} seconds.", start_time.elapsed().as_secs());

	let mut task_list : Vec<DisabledTimedTask> = Vec::new();
	...
	DisabledTimedTask::report( start_time, &mut task_list);
}
	...
	for k in start_ind..end_ind {
			...
			'outer: for j in 0..n_bams {
				...
				for indel in &pile.indels {
					if indel.sequence == indel_seq {
						// Output indel stats
						print!("\t{}:{}:{:.0}:{:.0}:{:.0}", indel.reads, pile.total, indel.mapq, indel.baseq, indel.sidedness );
		...
		for base in 0..4 {
			...
			for bam_index in 0..n_bams {
				...
				print!("\t{}:{}:{:.0}:{:.0}:{:.0}", reads, pile.total, acgt.mapq, acgt.baseq, acgt.sidedness);
...
fn count_indel(pileup: &mut Pileup, seq: &str, end_distance : f32, mapq : f32, baseq : f32 ) {
	for i in 0..pileup.indels.len() {
		...
		if &*indel.sequence == seq {
			...
			indel.baseq = (indel.baseq * n_reads_before + baseq) / n_reads;			
			return;
		}
	}
